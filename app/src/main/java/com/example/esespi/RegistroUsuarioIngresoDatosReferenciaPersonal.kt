...
class RegistroUsuarioIngresoDatosReferenciaPersonal : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_registro_usuario_ingreso_datos_referencia_personal)
...
//TEXTBOX--------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        txtNombre=findViewById(R.id.RegistroDatosReferenciaPersonalTxtNombre)
        txtApellido=findViewById(R.id.RegistroDatosReferenciaPersonalTxtApellido)
        txtDUI=findViewById(R.id.RegistroDatosReferenciaPersonalTxtDUI)
        txtCorreo=findViewById(R.id.RegistroDatosReferenciaPersonalTxtCorreo)
        txtTelefono=findViewById(R.id.RegistroDatosReferenciaPersonalTxtTelefono)
        txtDomicilio=findViewById(R.id.RegistroDatosReferenciaPersonalTxtDireccion)

        btnHombre=findViewById(R.id.RegistroDatosReferenciaPersonalBtnMasculino)
        btnMujer=findViewById(R.id.RegistroDatosReferenciaPersonalBtnFemenino)

        dbEstCivil=findViewById(R.id.RegistroDatosReferenciaPersonalDbEstadoCivil)
        dbTipoSangre=findViewById(R.id.RegistroDatosReferenciaPersonalDbTipoSangre)

        dbDia=findViewById(R.id.RegistroDatosReferenciaPersonalDbDia)
        dbMes=findViewById(R.id.RegistroDatosReferenciaPersonalDbMes)
        dbAño=findViewById(R.id.RegistroDatosReferenciaPersonalDbAño)

        dbDepartamento=findViewById(R.id.RegistroDatosReferenciaPersonalDbDepartamento)
        dbMunicipio=findViewById(R.id.RegistroDatosReferenciaPersonalDbMunicipio)

        linearLayoutIdiomas = findViewById(R.id.RegistroDatosReferenciaPersonalLinearLayoutIdiomasSeleccionados)
        linearLayoutNacionalidad = findViewById(R.id.RegistroDatosReferenciaPersonalLinearLayoutNacionalidadSeleccionados)
        btnAñadirNacionalidad=findViewById(R.id.RegistroDatosReferenciaPersonalbtnAñadirNacionalidad)
        btnAñadirIdioma=findViewById(R.id.RegistroDatosReferenciaPersonalbtnAñadirIdioma)
        btnSiguiente = findViewById(R.id.RegistroDatosReferenciaPersonalBtnSiguiente)
...
//DEPARTAMENTO Y MUNICIPIO--------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        dbDepartamento.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                ...
                val municipioAdapter = ArrayAdapter(this@RegistroUsuarioIngresoDatosReferenciaPersonal, android.R.layout.simple_spinner_item, municipios)
...
//RE INTEGRACION DE DATOS PARA EDICIÓN---------------------------------------------------------------------------------------------------------------------------------------
        ...
        if (userData != null) {
            ...
            txtApellido.setText(userData["apellido"])
            txtDUI.setText(userData["dui"])
            txtCorreo.setText(userData["correo"])
            txtTelefono.setText(userData["telefono"])
            txtDomicilio.setText(userData["domicilio"])

            val generotxt = userData["genero"]
            if (generotxt != null) {
                if (generotxt == "Masculino") {
                    genero = 1
                    btnHombre.backgroundTintList = ColorStateList.valueOf(Color.parseColor("#4399FF"))
                    btnHombre.setTextColor(Color.parseColor("#FFFFFF"))
            ...
            val estadoCivil = userData["estadoCivil"]
            if (estadoCivil != null) {
                val estadoCivilAdapter = dbEstCivil.adapter as ArrayAdapter<String>
                val position = estadoCivilAdapter.getPosition(estadoCivil)
                dbEstCivil.setSelection(position)
            }

            val tipoSangre = userData["tipoSangre"]
            if (tipoSangre != null) {
                val tipoSangreAdapter = dbTipoSangre.adapter as ArrayAdapter<String>
                val position = tipoSangreAdapter.getPosition(tipoSangre)
                dbTipoSangre.setSelection(position)
            }

            val fechaNacimiento = userData["fechaNacimiento"]
            if (fechaNacimiento != null) {
                val fechaSplit = fechaNacimiento.split("/")
                if (fechaSplit.size == 3) {
                    val dia = fechaSplit[0].toInt()
                    val mes = fechaSplit[1].toInt()
                    val año = fechaSplit[2].toInt()

                    val diaAdapter = dbDia.adapter as ArrayAdapter<String>
                    val diaPosition = diaAdapter.getPosition(dia.toString())
                    dbDia.setSelection(diaPosition)

                    val mesAdapter = dbMes.adapter as ArrayAdapter<String>
                    val mesPosition = mesAdapter.getPosition(mes.toString())
                    dbMes.setSelection(mesPosition)

                    val añoAdapter = dbAño.adapter as ArrayAdapter<String>
                    val añoPosition = añoAdapter.getPosition(año.toString())
                    dbAño.setSelection(añoPosition)
                }
            }

            val domicilio = userData["domicilio"]
            if (domicilio != null) {
                val domicilioSplit = domicilio.split(", ")
                if (domicilioSplit.size == 3) {
                    val dep = domicilioSplit[0]
                    val mun = domicilioSplit[1]
                    val di = domicilioSplit[2]

                    val departamentoAdapter = dbDepartamento.adapter as ArrayAdapter<String>
                    val depPosition = departamentoAdapter.getPosition(dep)
                    dbDepartamento.setSelection(depPosition)



                    txtDomicilio.setText(di)
                }
            }
        }
